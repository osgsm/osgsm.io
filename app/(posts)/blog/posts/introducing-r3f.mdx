...
次のステップでは、この値を使って `<mesh />` の `rotation` の値を変更します。
...
`useRef` を `React` からインポートし、 `Sphere` コンポーネントで使用します。このとき、 `useRef` には型を明示する必要があります（[Using with TypeScript - React Three Fiber](https://r3f.docs.pmnd.rs/tutorials/typescript#typing-with-useref) 参照）。
...
<Video src="/blog/images/introducing-r3f/video1.mp4" autoPlay loop controls={false} />
